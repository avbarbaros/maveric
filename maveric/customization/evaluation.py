...
class Evaluator(BaseComponent):
    ...
    def evaluate_multilabel_map(self,
                                model: nn.Module,
                                data_loader: Any,
                                class_names: List[str],
                                templates: List[str] = None,
                                use_ensemble: bool = True) -> Tuple[float, Dict[str, float]]:
        """
        Evaluate multi-label classification using Mean Average Precision (mAP).

        Used for datasets like VOC2007 where each image can belong to multiple classes.
        Labels are expected as multi-hot float tensors of shape (batch, num_classes).

        Args:
            model: Model to evaluate
            data_loader: Data loader (labels must be multi-hot float tensors)
            class_names: List of class names
            templates: List of prompt templates
            use_ensemble: Whether to use template ensembling

        Returns:
            Tuple of (mAP percentage, per_class_AP dict)
        """
        ...
        if use_ensemble and templates is not None:
            ...
            class_prompts = [f"a photo of a {name}." for name in canonical_names]
            with torch.no_grad():
                text_features_output = model.clip_model.get_text_features(
                    **model.processor(text=class_prompts, return_tensors="pt", padding=True).to(self.device)
                )
                if isinstance(text_features_output, torch.Tensor):
                    class_text_features = text_features_output
                else:
        ...
        all_scores = []   # (num_samples, num_classes) - raw similarity scores
        all_labels = []   # (num_samples, num_classes) - multi-hot ground truth

        with torch.no_grad():
            for images, texts, labels in tqdm(data_loader, desc="Evaluating (mAP)"):
                labels = labels.to(self.device)

                logits = model(images, class_text_features)  # (batch, num_classes)
        ...
        for i, class_name in enumerate(class_names):
            ...
            if all_labels[:, i].sum() > 0:  # skip classes with no positive samples
                ap = average_precision_score(all_labels[:, i], all_scores[:, i])
                per_class_ap[canonical_name] = ap * 100
                aps.append(ap)
            else:
                per_class_ap[canonical_name] = 0.0
