"""
04_unified_evaluation.py - Evaluate a unified model checkpoint per ELEVATER dataset.
...
Usage:
    python experiments/04_unified_evaluation.py \
...
def print_comparison_table(results: dict):
    ...
    for dataset_name in sorted(results.keys()):
        ...
        if baseline is None and custom is None:
            print(f"  {dataset_name:<18} {'N/A':>10} {'N/A':>12} {'N/A':>9}")
            continue

        b_str = f"{baseline:.2f}%" if baseline is not None else "N/A"
        c_str = f"{custom:.2f}%"   if custom   is not None else "N/A"

        if baseline is not None and custom is not None:
    ...
    if improvements:
        avg_b   = sum(r["baseline_accuracy"]   for r in results.values()
                      if r.get("baseline_accuracy") is not None) / max(1, sum(
                          1 for r in results.values() if r.get("baseline_accuracy") is not None))
        avg_c   = sum(r["customized_accuracy"] for r in results.values()
                      if r.get("customized_accuracy") is not None) / max(1, sum(
                          1 for r in results.values() if r.get("customized_accuracy") is not None))
...
def main():
    ...
    customizer.model = baseline_model.clip_model   # underlying CLIPModel
    ...
    for dataset_name in eval_datasets:
        ...
        c_acc, c_per_class, _ = evaluate_one_dataset(
            customized_model, dataset_name, class_names,
            evaluator, customizer, use_templates, args.detailed
        )
        if c_acc is None:
            print(f"   ⚠️  Could not evaluate customized model.")
            continue
    ...
    avg_baseline   = (sum(r["baseline_accuracy"]   for r in evaluated) / len(evaluated)) if evaluated else 0
    avg_customized = (sum(r["customized_accuracy"] for r in evaluated) / len(evaluated)) if evaluated else 0
    avg_improvement = avg_customized - avg_baseline
    ...
    output_data = {
        ...
        "summary": {
            ...
            "avg_customized_accuracy": round(avg_customized, 4),
